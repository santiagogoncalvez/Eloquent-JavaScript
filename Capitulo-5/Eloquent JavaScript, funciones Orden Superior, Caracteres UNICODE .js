...
// Funcion que filtra ciertos datos de un array, en esta caso agrega al array pasaron 
// todos los objetos que en su propiedad "living" contienen el valor true.
...
// La anterior funcion muestra como funciona la siguiente funcion por dentro,
...
let arrayy = ["a", "b", "a"];
let copia = arrayy.filter((elemento) => elemento == "a");
console.log(copia)




// Mapear, transformar un array

// Ahora hay que crear un array de los nombres de los codigos que queremos encontrar segun alguna propiedad
function map(array, transformar) {
  let mapeados = [];
  for (let elementos of array) {
    mapeados.push(transformar(elementos));
  }
...
console.log(codigosDerechaAIzquierda);
console.log(map(codigosDerechaAIzquierda, codigo => codigo.name))

// el metodo resumido de esta funcion es
// map() toma un array y le aplica a cada elemento una funcion dada y retorna un nuevo array
// con los elementos del array que retorna esa funcion
console.log(codigosDerechaAIzquierda.map(codigo => codigo.name));
...
// funcion/metodo resumido
// Si el array contiene al menos un elemento, se puede omitir el argumento inicio.
// Toma el primer valor del array como su valor de inicio y empieza a reducir desde el segundo.
console.log(arr.reduce((a, b) => a + b))




// Usar reduce para encontrar el codigo con mayores caracteres
// El proceso es el siguiente:
/* A cada elemento del array compararlo con el valor guardado, analizando si la cantidad de caracteres es mayor,
 y si es asi guardar el elemento, y seguir con la comparación */

function cuentaCaracteres(codigo) {
  return codigo.ranges.reduce((cuenta, [desde, hasta]) => { return cuenta + (hasta - desde); }, 0);
}

console.log(SCRIPTS.reduce((a, b) => { return cuentaCaracteres(a) < cuentaCaracteres(b) ? b : a }).name);

let han = SCRIPTS.reduce((a, b) => { return cuentaCaracteres(a) < cuentaCaracteres(b) ? b : a })
// console.log(han)
// console.log("Cantidad de caracteres: ", cuentaCaracteres(han))
// 'Hangul'

// La siguiente parte de texto extrae del objeto SCRIPTS el elemento con el nombre ingresado
let codigo = SCRIPTS.reduce((actual, element) => element.name == 'Arabic' ? element : actual).name;
// console.log(codigo);
// console.log(cuentaCaracteres(codigo));

function nombreSimbolo (SCRIPT, simbolo) {
  return SCRIPTS.reduce((actual, element) => element.name == 'Arabic' ? element : actual).name;
}
...
// Codigo que encuentra el año de origen promedio de los codigos vivos, y de los codigos muertos:
...
// La siguiente funcion sirve para encontrar el codigo al que pertenece un caracter:
...
// La siguiente funcion sirve para contar los caracteres que pertenecen a cada codigo
function contarPor(elementos, nombreGrupo) {
  let cuentas = [];
  for (let elemento of elementos) {
    let name = nombreGrupo(elemento);
    let conocido = cuentas.findIndex(c => c.name == name);
    if (conocido == -1) {
      cuentas.push({ name, count: 1 });
    } else {
      cuentas[conocido].count++;
    }
  }
...
console.log(contarPor([1, 2, 3, 4, 5, 6, 7, 8], n => n > 6));

// del primer elemento al sexto no cumplen la condicion y están
// en el grupo (objeto) de los falsos, los que no cumplen la condicion.
// Y los del septimo en adelante cumplen la condicion y estan en el grupo
// de los verdaderos.
...
// La siguiente funcion indica que codigos se utilizan en una pieza de texto:
function codigosTexto(texto) {
  // codigos va a almacenar un array con objetos que contengan los nombres y cantidades de cada tipo de caracter
  // que aparezca en el texto introducido
  ...
  // La vinculacion total va a almacenar un array reducido de codigos,
  // el cual va a contener la suma de la cantidad de caracteres que hay en el texto, distintos de "ninguno" (vacio)
  let total = codigos.reduce((n, { count }) => n + count, 0);
  console.log(total)
  if (total == 0) return "No se encontraron codigos";


  // Finalmente se va a retornar un string que contenga el porcentaje de cada tipo de lenguaje
...
console.log(codigoCaracter(110642).name)
console.log(String.fromCodePoint(110642))

console.log(codigosTexto("𓏀"));

console.log(String.fromCodePoint(43341) == "ꥍ")
...
// Array de ponderaciones
// Esta funcion devuelve un array con las ponderaciones crecientes de cada elemento de una cantidad n de elementos
function ponderacion(cantElementos) {
  let ponderacion = Number((1 / cantElementos).toFixed(3));
  let arrayPonderaciones = [];
  let acumulador = ponderacion;

  for (let i = 0; i < cantElementos; i++) {
    arrayPonderaciones.push(Number(acumulador.toFixed(10)));
    acumulador += ponderacion;
  }

  return arrayPonderaciones;
}
...
// Funcion que crea un array con los nombres de los codigos ordenados de menor a mayor segun cuantos caracteres posean

function ordenarLenguajesCantCaracteres(SCRIPTS) {
  let arrayNombres = [];

  function codigoMayorCaracteres(script) {
    return script.reduce((a, b) => { return cuentaCaracteres(a) > cuentaCaracteres(b) ? b : a });
  }
  ...
  let mayorCodigo = codigoMayorCaracteres(SCRIPTS);
  arrayNombres.push(mayorCodigo.name);
  let scriptRestante = SCRIPTS.filter(element => element.name != mayorCodigo.name);

  while (arrayNombres.length < SCRIPTS.length) {
    mayorCodigo = codigoMayorCaracteres(scriptRestante);
    arrayNombres.push(mayorCodigo.name);
    scriptRestante = scriptRestante.filter(element => element.name != mayorCodigo.name);
  }
...
// Funcion que copia los elementos de un array a otro array;
function copiarObjeto(objeto) {
  let copiaScripts = [];
  for (element of objeto) {
    copiaScripts.push(element);
  }
  return copiaScripts;
}

// Funcion que extrae el rango de cada lenguaje

function contarPor2(nombres, probabilidades, scriptMayor) {
  let cuentas = [];
  let contador = 0;
  for (let nombre of nombres) {
    let rango = scriptMayor.reduce((n, element) => element.name == nombre ? element : n).ranges;
    let conocido = cuentas.findIndex(c => c.rango == rango);
    if (conocido == -1) {
      cuentas.push({ rango, probabilidad: probabilidades[contador] });
    }
    contador++
  }
...
// calcular la probabilidad exacta de un elemento en un grupo de n elementos, donde la probabilidad del primer elemento es menor y las probabilidades aumentan gradualmente de menor a mayor
function calcularProbabilidades(cantidadElementos) {
  // Calcular la suma de los números desde 1 hasta cantidadElementos
  const sumaTotal = (cantidadElementos * (cantidadElementos + 1)) / 2;
  
  // Inicializar un array para almacenar las probabilidades
  const probabilidades = [];

  // Calcular la probabilidad de cada elemento y agregarla al array
  for (let i = 1; i <= cantidadElementos; i++) {
    const probabilidad = i / sumaTotal;
...
let probabilidades = calcularProbabilidades(139);
console.log(probabilidades)

let nombresCantidCaracteres = ordenarLenguajesCantCaracteres(SCRIPTS);
console.log(nombresCantidCaracteres)

let ponderacionesMenorMayor = invertirNuevo(probabilidades);
console.log(ponderacionesMenorMayor);

let arrayNombresProbabilidades = contarPor2(nombresCantidCaracteres, ponderacionesMenorMayor, SCRIPTS)
console.log(JSON.stringify(arrayNombresProbabilidades));



// Funcion que encuntra el ultimo o primer codigo UNICODE que pertenece a un lenguaje
function encontrarCodigo(SCRIPTS, funcionCondicion) {

  let array = [];
  for (element of SCRIPTS) {
    array.push(element.ranges.reduce((n, elemento) => n.concat(elemento)).reduce(funcionCondicion));
  }

  return array.reduce(funcionCondicion);
}

// console.log(encontrarCodigo(SCRIPTS, (actual, elemento) => elemento > actual ? elemento : actual))
...
{
  // function simboloAleatorioLeng(SCRIPTS, probabilidades) {
  //   // Este bucle devuelve un numero aleatorio que se encuentre dentro de algun rango de los codigos de algun lenguaje
  //   let numerAleatorio = Math.random();

  //   while (true) {
  //     let aleatorio = Math.floor(Math.random() * (195101 - 64 + 1)) + 64;

  //     if (verificarCaracter(SCRIPTS, aleatorio)) {
...
// function verificarCaracter(SCRIPTS, codigo_caracter) {
...
// }}
 ...
 function verificarCaracter (rangos, codigo_caracter) {
...
function simboloAleatorioLeng (probabilidades) {
  // Este bucle devuelve un numero aleatorio que se encuentre dentro de algun rango de los codigos de algun lenguaje
  let numeroAleatorio = Math.random() * 2;
  ...
  // while (true) {
      const numeroDentroDelRango = Math.floor((Math.random() * (rangoSeleccionado[rangoSeleccionado.length - 1] - rangoSeleccionado[0][0] + 1)) + rangoSeleccionado[0][0] );
      if (verificarCaracter(rangoSeleccionado, numeroDentroDelRango)) {
          // break;
      };
  // };
