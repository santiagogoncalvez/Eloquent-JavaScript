// Ejercicio direccion de escritura dominante, capitulo 4
...
// La siguiente funcion sirve para encontrar el codigo al que pertenece un caracter, retorna un objeto con el codigo
...
// Crear funcion que calcule la direccion dominante en un string:
function direccionDominante(texto) {

    // Formar los grupos de las direcciones de los caracteres con sus nombres y cantidades
let direcciones = contarPor(texto, caracter => {
    let codigo = codigoCaracter(caracter.codePointAt(0))
    return codigo ? codigo.direction : "ninguno";
}).filter(({name}) => name != "ninguno");

console.log(direcciones)
// Encontrar el mayor valor de las propiedades count (cantidad), que seria la direccion predominante
let mayoria = direcciones.reduce((n, {count}) => (n > count) ? n : count, 0);
console.log(mayoria)
// Buscar el objeto de direccion que tenga esa cantidad y devolver su nombre (direccion)
return direcciones.filter(element => element.count == mayoria).map(element => element.name);
}

console.log(direccionDominante("ぁ"));

console.log(SCRIPTS.filter(element => element.direction == "ttb").map(element => element.name))