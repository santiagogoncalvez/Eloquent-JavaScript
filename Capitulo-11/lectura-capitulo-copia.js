...
requestType("connections", (nest, externalConnections, source) => {
    let currentNestConnections = nest.state.connections;

    for (let connection of externalConnections) {
        let name = Object.keys(connection)[0];
        let neighbors = connection[name];

        // console.log(JSON.stringify(currentNestConnections.get(name)));
        // console.log(JSON.stringify(neighbors))

        if (
            JSON.stringify(currentNestConnections.get(name)) ==
            JSON.stringify(neighbors)
        ) {
            // console.log("Se continuó");
            continue;
        }
...
function broadcastConnections(nest, name, exceptFor = null) {
    ...
    let connectionsArr = Object.keys(connectionsObject).map((clave) => ({
        [clave]: connectionsObject[clave],
    }));

    for (let neighbor of nest.neighbors) {
        if (neighbor == exceptFor) continue;
        request(nest, neighbor, "connections", connectionsArr);
    }
}
...
// console.log(bigOak.state.connections.get('Hawthorn'));
